...
bool compositeOutInit(CompositeOutState *s, const CompositeTiming *timing,
  ...
  // PAL + PWM cannot afford the per-pixel DIRECT chroma paths: measured on
  // hardware (DWT slot timing), even with the USAT/no-clamp encoder, the
  // SRAM-resident fill path and the 319.2 MHz clock, the fill sits at ~95% of
  // the two-line slot budget and the OSD overlay pushes it past 100% -- the
  // ring then transmits stale slots and the picture tears whenever the menu
  // is open. PAL R-2R has a four-line slot (twice the jitter averaging) and
  // holds worst-case ~95% with the OSD open; NTSC has ~25% more cycles per
  // pixel and holds on both DACs. So this one combination falls back to the
  // RGB332 LUT path, which it can always afford.
  CompositeChromaMode chromaMode = cfg.chromaMode;
  if (timing->palSwitch && cfg.dac->kind == CompositeDacKind::PWM) {
    chromaMode = CompositeChromaMode::LUT;
  }

  // The DIRECT chroma modes use no LUT (lutWords == 0); don't malloc(0).
  const uint32_t lutWords = compositeLutWords(timing, cfg.dac, chromaMode);
  s->lut = nullptr;
  if (lutWords != 0u) {
    s->lut = (uint32_t *)malloc(lutWords * sizeof(uint32_t));
    if (s->lut == nullptr) return false;
  }
  if (!compositeEncoderInit(&s->enc, timing, cfg.dac, s->lut, chromaMode))
    return false;
