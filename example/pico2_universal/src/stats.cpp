...
static void sampleRaw(RawCounters* c) {
  ...
  switch (gSrc.outputIf) {
    case OutputInterface::NTSC:
    case OutputInterface::PAL:
      c->txUnderrun = gSrc.cvbs->underrunCount;
      c->txFrames = gSrc.cvbs->frame;
      break;
    case OutputInterface::DISPLAY_LINK:
      // The adapter self-refreshes; there is no line-synchronous underrun.
      c->txUnderrun = 0;
      c->txFrames = gSrc.displaylink->frameCount;
      break;
    default:
      c->txUnderrun = gSrc.hstx->underrunCount;
      c->txFrames = gSrc.hstx->frame;
      break;
  }
...
int statsCollect(lcdtap::StatEntry* out, int maxCount) {
  ...
  const lcdtap::StatEntry entries[] = {
      ...
      {"RX HW Overflow", c.rxHwOverflow - gBase.rxHwOverflow, "",
       StatFormat::DEC},
      {"TX Underrun", c.txUnderrun - gBase.txUnderrun, "", StatFormat::DEC},
      {"TX Frames", c.txFrames - gBase.txFrames, "", StatFormat::DEC},
...
void statsReset() {
  ...
  // apply; both fields are only ever written from the Core 0 main loop, so
  // clearing them here is race-free.
