...
- **セルフテスト** (`I2C_SELFTEST_ENABLE`): G47→G53 (SDA)、G48→G54 (SCL)
  をジャンパ接続すると、規格タイミング (~50 kHz) のビットバングマスタが
  5 秒毎に既知の SSD1306 シーケンスを送信し、マスタ側 ACK 数を
  `[selftest]` 行に、スレーブ側の受信結果を通常の診断に出力する。
  外部マスタ無しでスレーブ単体を検証できる。
